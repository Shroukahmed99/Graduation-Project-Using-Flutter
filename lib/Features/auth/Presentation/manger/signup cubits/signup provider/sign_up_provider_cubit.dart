...
class SignUpProviderCubit extends Cubit<SignUpState> {
  final UsersRepo usersRepo;
  GlobalKey<FormState> formKey = GlobalKey<FormState>();

  SignUpProviderCubit(this.usersRepo) : super(SignupInitial());
  ...
  Future<void> signUpUser() async {
    ...
    String jobTitle = CacheHelper.getData(key: 'jobTitle') ?? '';
    String bio = CacheHelper.getData(key: 'bio') ?? '';
    String priceRange = CacheHelper.getData(key: 'priceRange') ?? '';

    // استرجاع ملف الـ identifier (بشكل اختياري)
    File identifier = (CacheHelper.getData(key: 'identifier') ?? '');
